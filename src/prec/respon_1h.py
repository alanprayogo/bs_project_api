...
def prec_respon_1h(hand):
    ...
    # 1NT
    if 6 <= total_hcp <= 11 and (analyzer.is_balanced_distribution(dist) and suit_counts["H"] <= 2):
        possibleBids.append(2)
    # 2C
    if total_hcp >= 12 and (suit_counts["C"] >= 5 and suit_counts["H"] <= 2):
        possibleBids.append(3)
    # 2D
    if total_hcp >= 12 and (suit_counts["D"]>= 5 and suit_counts["H"] <=2):
